...
def homography_regression_model_transfer_learning(lr):
    ...
    for layer in modelMobileNet1.layers:
        if (i < 130): # en total son 156, frizamos solo las primeras 130
    ...
    x = Dense(1024, name='fc1', activation='relu',kernel_regularizer=tf.keras.regularizers.l2(0.01))(merged)
    x = Dropout(0.5)(x)
    ...
    model.compile(
        ...
        metrics=['mse', mace])
    print("optimizer=SGD(lr="+str(lr)+", momentum=0.9)")
...
if __name__ == "__main__":
    ...
    model = homography_regression_model_transfer_learning(0.01)
